...
graph TB
    subgraph "搜索系统"
        SearchStore["searchStore<br/>(~300 行)"]
...
该 store 负责 VRC API 驱动的搜索、直接实体访问和剪贴板直接访问。旧的 store 内快速搜索（`quickSearchRemoteMethod`）已在 `d52b0c7c` 中**删除** —— 所有客户端快速搜索现在完全由 `quickSearchStore` + `quickSearchWorker` 处理。
...
| 文件 | 行数 | 用途 |
|------|------|------|
| `stores/search.js` | ~300 | 直接访问、用户搜索 API |
